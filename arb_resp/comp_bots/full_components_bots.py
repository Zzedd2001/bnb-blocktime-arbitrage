...
ROOT = ROOT + "/arb_resp/comp_bots"; OUT = os.path.join(ROOT, "out"); os.makedirs(OUT, exist_ok=True)
...
KEY = {}
S_SAMPLE = json.load(open(ROOT + "/arb_resp/cmp_bots/key_numbers.json"))          # sample-based results (R2 τ-implied, R3c)
...
def load(f, p):
    d = pd.read_csv(os.path.join(ROOT, f, "arb_response_bots", "component_panels", f"{p}.csv"))
...
rows = []; C4 = {}
for f in FORKS:
    W = json.load(open(os.path.join(ROOT, f, "arb_response_bots", "component_panels", "component_summary.json")))["weights"]
    for p in CORE:
        w = W[p]["14d"]
