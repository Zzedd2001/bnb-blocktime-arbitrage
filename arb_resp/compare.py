...
ROOT = ROOT + "/arb_resp"; OUT = os.path.join(ROOT, "cmp"); os.makedirs(OUT, exist_ok=True)
...
def load(fork):
    S = json.load(open(os.path.join(ROOT, fork, "arb_response", "summary.json")))["pools"]
    samples = {}
    for p in S:
        f = os.path.join(ROOT, fork, "arb_response", p, "arbs_sample.parquet")
        if os.path.exists(f):
...
for f in FORKS:
    ...
    for p in CORE:
        ...
        pr = pd.read_csv(os.path.join(ROOT, f, "arb_response", p, "paired_senders.csv"))
...
for f in FORKS:
    for p in CORE:
        for reg in ("pre", "post"):
            t = pd.read_csv(os.path.join(ROOT, f, "arb_response", p, f"top_senders_{reg}.csv")).head(5)
